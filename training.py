...
experiments = [
    (ReadM4(m4_key), ZScoreNormalization((0,), 3, 2, [True, True, True, False], min_group_size=0,
     ...
     PWN(config, config_c, train_spn_on_gt=False, train_spn_on_prediction=True, train_rnn_w_ll=False, smape_target= True,
            always_detach=True),
...
for i, (data_source, preprocessing, model, evaluation_metrics, plots, load, use_cached) in enumerate(experiments):
    ...
    if not use_cached:
        ...
        train_x, train_y,test_x_aux,test_y_aux, test_x, test_y, column_names, embedding_sizes, last_sequence_labels = \
        ...
        test_y_aux_values = {key: y[..., -1] if len(y) > 0 else y for key, y in test_y_aux.items()}
    ...
    if load is None:
        ...
        model.train(train_x, train_y, test_x_aux, test_y_aux, embedding_sizes,epochs=100)
    ...
    test_x_aux = {key: val for key, val in test_x_aux.items() if len(val) > 0}

    test_y = {key: val for key, val in test_y.items() if len(val) > 0}
    test_y_aux = {key: val for key, val in test_y_aux.items() if len(val) > 0}

    test_y_values = {key: val for key, val in test_y_values.items() if len(val) > 0}
    test_y_aux_values = {key: val for key, val in test_y_aux_values.items() if len(val) > 0}

    predictions = model.predict(test_x, mpe=plots['mpe'], pred_label='test_pred')
    predictions_aux = model.predict(test_x_aux, mpe=plots['mpe'], pred_label='test_pred_aux')
    ...
    if type(predictions) is tuple:
        if plots['mpe']:
            ...
            predictions_aux, likelihoods_aux = predictions_aux
            if plots['train']:
    ...
    index_test_aux = {key: np.array([list(range(p.shape[-1]))]) for key, p in predictions_aux.items()}
    if plots['train']:
    ...
    if plots['reversed']:
        ...
        predictions_reversed_aux = preprocessing.reverse(predictions_aux)
        test_y_values_reversed = preprocessing.reverse(test_y_values)
        test_y_aux_values_reversed = preprocessing.reverse(test_y_aux_values)
        if plots['train']:
    ...
    for eval_metric in evaluation_metrics:
        ...
        result_test_aux = eval_metric.calculate(predictions_reversed_aux, test_y_aux_values_reversed, likelihoods_aux) \
            if plots['reversed'] else eval_metric.calculate(predictions_reversed_aux, test_y_aux_values_reversed, likelihoods_aux)
        #eval_results[metric_name] = result_test
        eval_results[metric_name] = result_test_aux
        ...
        if type(eval_metric) in (MSE, SMAPE) and plots['mpe']:
            try:
                if reversed:
                    ...
                    mpe_error = eval_metric.calculate(mpe_pred_reversed, test_y_aux_values_reversed, likelihoods)
                else:
                    mpe_error = eval_metric.calculate(predictions_mpe, test_y_aux_values, likelihoods)
                ...
                if plots['train']:
                    if reversed:
                        ...
                        mpe_error_train = eval_metric.calculate(train_predictions_mpe, train_y_aux_values, None)
        ...
        if plots['train']:
            result_train = eval_metric.calculate(train_predictions_reversed, train_y_aux_values_reversed,
